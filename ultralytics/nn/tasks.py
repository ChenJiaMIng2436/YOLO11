...
from ultralytics.nn.core11 import *
import contextlib
import pickle
import re
import types
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {
                ...
                n = 1
            if m is C3k2 and scale in "lx":  # for M/L/X sizes
                args[3] = True
        elif m is AIFI:
            args = [ch[f], *args]
...
# -------------------------------------------------
            ...
            if m in [C3_RMB, CSRMBC, C2f_RMB, CPNRMB]:
                args.insert(2, n)  # number of repeats
                n = 1
        elif m in [CS_Biformer, ReNLANBiF, CPNBiF, C3_Biformer, C2f_Biformer]:
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if not output
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
            if m in [CS_Biformer, CPNBiF, C3_Biformer, C2f_Biformer]:
                args.insert(2, n)  # number of repeats
                n = 1
        # -------------------------------------------------
        # ✅ 自定义 BiFormerBlock 模块解析逻辑
        # -------------------------------------------------
        elif m in [BiFormerBlock]:
            c1 = ch[f]                    # 输入通道
            c2 = args[0]                  # 输出通道（YAML 中第一个参数）
            n_win = args[1] if len(args) > 1 else 8   # 窗口大小（第二个参数，可选）
            
            # 通道数适配
            if c2 != nc:
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            
            # 参数格式匹配 BiFormerBlock(c1, c2, n_win)
            args = [c1, c2, n_win]
            # n 保留为重复次数，不需要修改
